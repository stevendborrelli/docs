...
Generating TypeScript models starts a Docker container running a Node.js
toolchain, so unlike the other languages it isn't generated by default. Add
`typescript` to `spec.schemas.languages` in `crossplane-project.yaml`:
